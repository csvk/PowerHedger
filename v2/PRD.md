...
- **Trimming Fund Sources**:
    1. **Initial Locked Profit (Immediate)**: When an active trade first reaches `LockProfitPips` and sets its Trailing SL, a portion (`KeepProfitPercent`) of the secured profit is immediately used for a trim.
    2. **Booked Delta Profit (At Close)**: When an active trade closes, the delta between its total realized profit and the amount already harvested at the "Lock" moment is calculated. A portion (`KeepProfitPercent`) of this delta is added to `ProfitTally`.
    3. **Trim Leg Profit (Harvested)**: When a hedged pair is trimmed, a portion (`KeepProfitPercent`) of the profit from the winning leg is added to `ProfitTally`.
- **Conditional Harvesting**: Profit is ONLY added to `ProfitTally` or used for immediate trims if there is at least one **Locked Sequence** on the chart at that exact moment.
