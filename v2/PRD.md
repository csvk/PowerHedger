...
- **Trimming Fund Sources**:
    1. **Booked Profit (At Close)**: When an active trade closes in profit, the total realized profit is calculated. A portion (`KeepProfitPercent`) of this profit is added to `ProfitTally` and immediately used to attempt a trim.
